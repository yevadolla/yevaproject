...
public class MessageStatsReport extends Report implements MessageListener {
	...
	public void done() {
		...
		double deliveryUtility = Double.NaN; // delivery utility
		...
		if (this.nrofResponseReqCreated > 0) {
			responseProb = (1.0* this.nrofResponseDelivered) / 
                ...
                if (this.nrofRelayed > 0) {
            deliveryUtility = (1.0 * this.nrofReceived) / this.nrofRelayed;
        }
		...
		String statsText = "created: " + this.nrofCreated + 
			...
			"\noverhead_ratio: " + format(overHead) + 
                        "\ndelivery_utility: " + format(deliveryUtility)+
			...
			"\nrtt_med: " + getMedian(this.rtt) +
                        "\nnilai_max_hobcount: " + getIntMax(this.hopCounts) +
                        "\nlist_hobcount: " +getjmlIntMax(this.hopCounts)
			;
